...
namespace Nhom3.Tests
{
    public abstract class Nhom3TestBase : AbpIntegratedTestBase<Nhom3TestModule>
    {
        ...
        protected void UsingDbContext(Action<Nhom3DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Nhom3DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Nhom3DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Nhom3DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Nhom3DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Nhom3DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Nhom3DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Nhom3DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Nhom3DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Nhom3DbContext, Task<T>> func)
        {
            T result;
